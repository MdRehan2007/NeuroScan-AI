...
├── best_cscan_model.pth       # download separately (see Section 2)
├── database.db                # created automatically on first run
├── requirements.txt
│
├── models/                    # CSCAN architecture
│   ├── __init__.py
│   ├── cscan.py
│   ├── convnext.py
│   ├── swin_transformer.py
...
The trained model is required for the website to make predictions.

The model file is not included directly in this repository because of its
large file size.
...
After downloading, place the file directly inside the CSCAN_WebApp folder:
...
Important: The website will not be able to make predictions if
best_cscan_model.pth is missing.
...
loaded on top. If those weights are already cached on this machine, the app
starts fully offline. Otherwise the first startup needs internet access to
download them once.
...
5. Using the app
Main page (/) — upload a JPG/JPEG/PNG brain MRI image, then click
Predict. The predicted tumor class and confidence percentage are
displayed.
History page (/history) — lists previous predictions with filename,
predicted class, confidence, and date/time.
6. How predictions work
The uploaded MRI image is validated and saved to static/uploads/.
The image is converted to RGB and passed through the validation/test
transform pipeline from utils/transforms.py.
The model runs in eval() mode.
If CSCANConfig.USE_TTA is True, the prediction uses the original image
and its horizontal flip.
The class with the highest probability is returned along with its
confidence.
The classes are:
glioma
meningioma
...
7. Troubleshooting
Problem	Fix
"Model not loaded"	Make sure best_cscan_model.pth is downloaded and placed in the project root.
Model file not found	Check that the file name is exactly best_cscan_model.pth.
ModuleNotFoundError	Run pip install -r requirements.txt.
Predictions seem slow	CPU is slower; a CUDA-enabled GPU can speed up prediction.
...
### Only thing you need to replace
...
to your actual Google Drive / Hugging Face / other model download link.

That way, a beginner opens your GitHub → downloads the model → puts it in the project folder → runs python app.py → website predicts.
